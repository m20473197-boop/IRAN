...
class JobRepository:
    ...
    async def list_active(self) -> list[Job]:
        stmt = select(Job).where(Job.is_active.is_(True)).order_by(Job.required_level, Job.salary)
        result = await self._session.execute(stmt)
        return list(result.scalars().all())

    async def list_all(self) -> list[Job]:
        stmt = select(Job).order_by(Job.required_level, Job.salary)
        result = await self._session.execute(stmt)
        return list(result.scalars().all())
    ...
    async def ensure_initial_jobs(self) -> list[Job]:
        ...
        existing_names = {job.name for job in existing}

        for job_data in initial_jobs:
            if job_data["name"] not in existing_names:
