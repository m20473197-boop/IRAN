...
def test_main_menu_exposes_only_implemented_features():
    ...
    # No fake buttons for future systems (market, crime, vehicles, ...).
    # Housing is now implemented, so «خانه» is allowed.
    labels = " ".join(b.text for b in buttons).lower()
    for banned in ("بازار", "جرم", "وسیله", "fromid", "market", "crime", "vehicle"):
